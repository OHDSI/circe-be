...
FROM
(
  select P.person_id, P.start_date, P.end_date, row_number() OVER (PARTITION BY person_id ORDER BY start_date @EventSort) ordinal, cast(P.visit_occurrence_id as bigint) as visit_occurrence_id
  FROM 
  (
  @criteriaQueries
  ) P
